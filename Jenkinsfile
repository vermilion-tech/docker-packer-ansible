pipeline {
    ...
    environment {
        ...
        DOCKER_REPO = "kadenlnelson/packer-ansible"
    ...
    stages {
        stage('Notify Slack') {
            ...
            steps {
                ...
                slackSend (color: '#0db7ed', message: "Docker Image Built & Pushed - https://hub.docker.com/r/kadenlnelson/packer-ansible/tags\n```\nTry it out!\n\ndocker run --rm ${DOCKER_REPO}:${COMMIT_HASH}```")
            }
        }
    }
